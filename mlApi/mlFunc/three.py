...
def box_fun(gray_img, color_img):
    (cnts, _) = cv2.findContours(gray_img,
...
def contours():
    ...
    original_bb_directory = os.path.join(
        settings.BASE_DIR, 'media', 'contours')
    ...
    for original_file_name in tqdm(original_file_list):
        ...
        (thresh, mask_img) = cv2.threshold(
            mask_img, 127, 255, cv2.THRESH_BINARY)
        ...
        new_name = os.path.join(
            original_bb_directory, new_name[0]+"_"+str(parasite_count)+"_parasites.png")
